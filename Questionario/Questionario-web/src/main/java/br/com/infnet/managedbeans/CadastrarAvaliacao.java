...
public class CadastrarAvaliacao implements Serializable{
	...
	public void init(){
		try {
			...
			for(OPCAO opcao : OPCAO.values()){
				SelectItem selectItem = new SelectItem(opcao,opcao.getStringValue());
	...
	public void cadastrarAvaliacao(){
		...
		try {
			...
			facesContext.addMessage("formCadastroAvaliacao", new FacesMessage(FacesMessage.SEVERITY_ERROR, null, "Ocorreu um Problema"));
			log.error(e.getMessage(),e);
		}
	}
